...
function Dashboard() {
  ...
  const handleAddClientChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === 'firstName' || name === 'lastName' || name === 'feeInterval' || name === 'preferredCurrency' || name === 'feePercentage') {

      setAddClientForm(prev => ({
        ...prev,
        profile: {
          ...prev.profile,
          [name]: name === 'feePercentage' ? processedValue : value
        }
      }));
  ...
  useEffect(() => {
    ...
    const fetchReports = async () => {
      setLoading(true);
      try {
        const res = await api.get(`/reports/all/${dateRange.from}/${dateRange.to}`, {
          headers: authHeaders,
        });
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      setLoading(true);
      try {
        const res = await api.get(`/history/all/${dateRange.to}`, {
          headers: authHeaders,
        });
        if (res.status >= 200 && res.status < 300) {
  ...
  if (auth.role === 'admin') {
    async function handleAddClientSubmit(event: React.FormEvent<HTMLFormElement>) {
      ...
      try {
      const res = await api.post(`/users`, addClientForm, {
        ...
        // Refetch clients
        const clientsRes = await api.get(`/users?role=client`, { headers: authHeaders as any });
        const clientsData = await clientsRes.data;
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%' }}>
                          <label style={{ fontWeight: 600, color: '#020202ff', minWidth: 90 }}>
                            ...
                            <input
                              ...
                              max={0.01}
                              step={0.0001}
                              value={addClientForm.profile.feePercentage ?? 0.003}
                              onChange={handleAddClientChange}
...
const UserReportsDashboard = ({
  ...
  useEffect(() => {
    if (!auth?.token) return;
    setLoading(true);
    api.get(`/reports/myinfo/${dateRange.from}/${dateRange.to}`, {
      headers: { Authorization: `Bearer ${auth.token}` },
    })
      .then((res) => res.data)
      .then((data) => {
    ...
    if (!auth?.token) return;
    setLoading(true);
    api.get(`/history/myinfo/${dateRange.to}`, {
      headers: { Authorization: `Bearer ${auth.token}` },
    })
      .then((res) => res.data)
      .then((data) => {
            ...
            <div style={{
              ...
              <div>
                ...
                <button
                  ...
                  onClick={async () => {
                  if (!selectedReport) return;
                  const res = await api.get(`/reports/${selectedReport.id}/pdf`, {
                    headers: { Authorization: `Bearer ${auth.token}` },
                    responseType: 'blob',
                  });
                  const blob = new Blob([res.data], { type: 'application/pdf' });
